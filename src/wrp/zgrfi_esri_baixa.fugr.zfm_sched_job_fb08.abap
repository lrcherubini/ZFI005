FUNCTION zfm_sched_job_fb08.
*"----------------------------------------------------------------------
*"*"Interface local:
*"  IMPORTING
*"     VALUE(IV_ANFBU) TYPE  BSEG-ANFBU
*"     VALUE(IV_ANFBN) TYPE  BSEG-ANFBN
*"     VALUE(IV_ANFBJ) TYPE  BSEG-ANFBJ
*"     VALUE(IV_STGRD) TYPE  STGRD DEFAULT '01'
...
* Get Print Parameters
  ...
  ev_jobname = |FB08_ESC_BXA_{ iv_anfbu }{ iv_anfbn }{ iv_anfbj }|.
  ...
  SUBMIT zfi0001 VIA JOB ev_jobname NUMBER ev_jobcount
    WITH p_anfbu = iv_anfbu
    WITH p_anfbn = iv_anfbn
    WITH p_anfbj = iv_anfbj
    WITH p_stgrd = iv_stgrd
    WITH p_bldat = iv_bldat
    AND RETURN.
